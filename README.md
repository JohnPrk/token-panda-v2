...
[![Download .dmg](https://img.shields.io/badge/Download-.dmg%20v1.74.8-6b4cff?style=for-the-badge&logo=apple)](https://github.com/JohnPrk/token-panda/releases/latest/download/token-panda_1.74.8_aarch64.dmg)
[![Download Windows](https://img.shields.io/badge/Download-Windows%20v1.74.8-0078d4?style=for-the-badge&logo=windows)](https://github.com/JohnPrk/token-panda/releases/latest)
...
| 버전 | 날짜 | 주요 변경 |
| --- | --- | --- |
| **1.74.x** | 2026-05-21 | **Windows 지원 (MVP).** Electron 으로 dev/runtime 마이그레이션, Tauri 기반 NSIS .exe 릴리스 파이프라인 유지. 초기 회귀 다수 수정 — 흰 화면(hash 라우팅 / Tauri 윈도우 라벨 라우팅 / store init hang), 트레이 항목 누락, 시작하기 데드락(블로킹 HTTP → async + spawn_blocking), 시작하기·설정 창 라벨 주입 레이스 |
